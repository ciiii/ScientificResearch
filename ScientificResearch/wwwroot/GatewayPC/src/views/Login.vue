...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        dbKey: [{ required: true, message: "请输选择医院！", trigger: "blur" }]
      },
      isDownload: false,
      key: ""
    };
  ...
  methods: {
    ...
    getHospita: async function() {
      this.hospitaList = await this.$http.myGet(
        URL_HOSPITAL.GET_ENABLE_HOSPITA,
        ""
      );
    },
    dataPost: async function() {
      ...
      //2019-7-13 在pc入口登录ok之后就设置isEntryLogin = true
