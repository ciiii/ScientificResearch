<template>
  <div class="app-container page-common">
    <h4 class="page-title" align="center">{{typeName}}列表</h4>
    <div class="filter-container">
      <el-button class="filter-item" type="success" @click="btnAddEdit(true,'')">
        <i class="iconfont icontianjia"></i> 添加{{typeName}}
      </el-button>
  ...
  export default {
    ...
    mounted() {
      if (this.$route.query.type) {
        this.req.教学活动类型编号 = this.$route.query.type;
      }
      this.getTypes();
      this.getList();
    ...
    watch: {
      $route(to, from) {
        this.req.教学活动类型编号 = this.$route.query.type;
        this.getContent(this.req.教学活动类型编号);
        this.$route.meta.title = this.typeName;
        this.getList();
      }
    },
    methods: {
      ...
      getContent(id) {
        this.types.find((item) => {
          if (item.编号 == id) {
            this.typeContent = item.活动内容模板;
            this.$route.meta.title = item.名称;
            return this.typeName = item.名称;
          }
        });
      },
