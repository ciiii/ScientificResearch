<template>
  <div class="app-container page-common page-addStudent" v-if="isLoad">
    ...
    <p class="item-title"><i class="iconfont iconkeshikucunguanli"></i><span> {{form.教学专业名称}} -- {{form.科室名称}}</span></p>
    ...
    <div class="content">
      <el-table class="tableone" ref="multipleTable" v-loading="listLoading" :data="list" element-loading-text="Loading"
        ...
        <el-table-column type="selection" align="center" width="55"  :reserve-selection="true" prop="编号"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="isDialog" v-if='isDialog'
      ...
      <addOrEditTask ref="child" @myEvent="getMyEvent" :item="item" :isAdd="isAdd" :info="form"
                     :id="this.req.教学专业科室编号"></addOrEditTask>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      if (sessionStorage.getItem('item')) {
        ...
        this.req.教学专业科室编号 = this.form.编号;
      }
    ...
    methods: {
      getTasks: async function() {
        let data = await this.$http.myGet(URL_SETTING.GET_PAGING_MAJOR_DEPARTMENT_TASK, this.req);
      ...
      postBatchDelTask: async function(data) {
        await this.$http.myPost(URL_SETTING.POST_DELETE_MAJOR_DEPARTMENT_TASK, data);
