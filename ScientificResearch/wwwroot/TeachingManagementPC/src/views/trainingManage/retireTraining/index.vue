<template>
  <div class="app-container page-common">
    <h4 class="page-title" align="center">学员退培审核</h4>
    <div class="filter-container">
      <el-input v-model="req.Like姓名" placeholder="请输入学员姓名" style="width: 180px;" class="filter-item"
    ...
    <div class="content" v-if="list&&list.length>0">
      <el-table class="tableone" v-loading="listLoading" :data="list"
        ...
        <el-table-column label="学员类型" prop="学员类型名称" align="center"></el-table-column>
        <el-table-column label="培训专业" prop="专业名称" align="center"></el-table-column>
        <el-table-column label="送培方式" prop="送培方式" align="center"></el-table-column>
        <el-table-column label="培训年届(年限)" prop="技能考试成绩" align="center">
          <template slot-scope="scope">
            {{scope.row.学员培训年界}}（ {{scope.row.学员培训年限}} 年）
          </template>
        </el-table-column>
        <el-table-column label="申请退培日期" align="center">
          <template slot-scope="scope">{{scope.row.申请退培日期|dataFormat('yyyy-MM-dd')}}</template>
        </el-table-column>
        <el-table-column label="学员状态" align="center">
          <template slot-scope="scope">
            <el-tag :type="getStudentStates(scope.row.状态)" size="small">{{scope.row.状态}}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="当前步骤" align="center">
          <template slot-scope="scope">
            <span :class="getStates(scope.row.LastStepStateName)">{{scope.row.LastStepTemplateName}}-{{scope.row.LastStepStateName}}</span>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      getList: async function() {
        let data = await this.$http.myGet(URL_WORK_FLOW.GET_PAGING_STUDENT_EXIT, this.req);
