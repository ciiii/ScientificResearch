<template>
  <div class="content-box">
    <el-form ref="form" :model="form" :rules="rules" label-width="130px" size="small">
      ...
      <el-form-item label="带教老师" prop="带教老师编号">
        <el-select v-model="form.带教老师编号" placeholder="请选择">
          <el-option
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          带教老师编号: [
            { required: true, message: '请选择带教老师！', trigger: 'blur' }, { validator: isInteger }
          ]
        },
        state: '',
        isLoad: false,
    ...
    methods: {
      ...
      confirmHandler: _debounce(function() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            this.addStudentEntry(this.form);
          } else {
      ...
      addStudentEntry: async function(data) {
        await this.$http.myPost(URL_TEACHING_MANAGEMENT.POST_STUDENT_ENTRY, data);
