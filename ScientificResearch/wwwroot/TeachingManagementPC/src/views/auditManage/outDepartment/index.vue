<template>
  <div class="app-container page-common">
    <h4 class="page-title" align="center">学员出科审核</h4>
    ...
    <div class="content" v-if="list&&list.length>0">
      <el-table class="tableone" v-loading="listLoading" :data="list"
        ...
        <el-table-column label="申请出科日期" align="center">
          <template slot-scope="scope">{{scope.row.申请出科日期|dataFormat('yyyy-MM-dd')}}</template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      getList: async function() {
        let data = await this.$http.myGet(URL_WORK_FLOW.GET_PAGING_OUT_DEPARTMENT, this.req);
