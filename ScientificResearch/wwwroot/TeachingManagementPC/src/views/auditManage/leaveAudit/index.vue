<template>
  <div class="app-container page-common">
    <h4 class="page-title" align="center">学员请假审核</h4>
    ...
    <div class="content" v-if="list&&list.length>0">
      <el-table class="tableone" v-loading="listLoading" :data="list"
        ...
        <el-table-column label="请假天数(天)" prop="请假天数" align="center"></el-table-column>
        <el-table-column label="请假开始日期" align="center">
          <template slot-scope="scope">{{scope.row.请假开始日期|dataFormat('yyyy-MM-dd')}}</template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      getList: async function() {
        let data = await this.$http.myGet(URL_WORK_FLOW.GET_PAGING_LEAVE_APPLY, this.req);
