      ...
      <el-tab-pane label="基本信息">
        <div class="item-box">
          ...
          <table class="mailTable">
            ...
            <tr>
              <th>申请退培日期</th>
              <td>{{form.申请退培日期|dataFormat('yyyy-MM-dd')}}</td>
              <th>退培日期</th>
              <td>{{form.退培日期|dataFormat('yyyy-MM-dd')}}</td>
            </tr>
          </table>
        </div>
      </el-tab-pane>
  ...
  export default {
    ...
    methods: {
      ...
      getDetails: async function(id) {
        ...
        let data = await this.$http.myGet(URL_WORK_FLOW.GET_STUDENT_EXIT_DETAILS, postData);
        if (data) {
          this.form = data;
        }
        this.isLoad = true;
