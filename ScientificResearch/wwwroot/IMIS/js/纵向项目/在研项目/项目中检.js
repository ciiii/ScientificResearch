    ...
    avalon.ready(function () {
        window.vm = avalon.define({
            ...
            query: function () {
                vm.loaded = false;
                $.support.cors = true;
                ResearchProject.getYProjectMiddleInspection('get', vm.req.$model, function getYProjectMiddleInspectionListener(success, obj, strErro) {
                    if (success) {
                        vm.loaded = true;
                        vm.total = obj.total;
            ...
            details: function () {
                $('.modal-details .detailsPage').attr('src', vm.getUrl('/IMIS/views/纵向项目/在研项目/纵向项目中检详情.html'));
            },
            ...
            confirm: function () {
                vm.addYProjectMiddleInspection(vm.data);
            },
            addYProjectMiddleInspection: function (data) {
                ResearchProject.addYProjectMiddleInspection('post', data, function addYProjectMiddleInspectionListener(success, obj, strErro) {
                    postBack(success, strErro, '提交成功！', '提交失败：', '.modal-central-inspection', function callBack() {
                        vm.query();
                    });
                });
            },
