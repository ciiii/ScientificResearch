    ...
    avalon.ready(function () {
        window.vm = avalon.define({
            ...
            query: function () {
                vm.loaded = false;
                $.support.cors = true;
                Award.getAwardList('get', vm.req.$model, function getAwardListListener(success, obj, strErro) {
                    if (success) {
                        vm.loaded = true;
                        vm.total = obj.total;
            ...
            search: function () {
                vm.req.Index = 1;
                vm.req.Like获奖名称 = vm.title;
                vm.req.Like第一完成人 = vm.name;
                vm.req.Begin获奖日期 = vm.startTime;
                vm.req.End获奖日期 = vm.endTime;
                vm.query();
            },
            submit: function () {
                if (event.keyCode == 13) {
        ...
        $('.modal-add .tab-paney').mCustomScrollbar({
            theme: 'dark-3',
        });
        vm.search();
