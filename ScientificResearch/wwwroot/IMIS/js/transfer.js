        ...
        initoption: function () {
           ...
           * 两个table渲染内容若一样，给任意一个columns即可*/
            ...
            * 两边table两边渲染选中的field不能一样，此处强制替换了，参数中可不写field*/
            this.option.selectColumns[0].checked = this.option.diffKey + 's';
            this.option.unselectColumns[0].checked = this.option.diffKey;
        },
        /*
        * 从数据中挑出已选列表和待选列表的数据*/
            ...
            if (type) {
                ...
                if ((this.option.maxSelect - 0) && typeof (this.option.maxSelect - 0) == "number") {
                    ...
                    if (currenNum > this.option.maxSelect) {
                        alert(this.selectTitle + '最多只能添加' + this.option.maxSelect + '个');
                        return false;
                    }
                }
                for (var i = 0; i < this.unselectData.length; i++) {
                    if (selectList.indexOf(this.unselectData[i][this.option.uniqueId]) >= 0) {
                        this.unselectData[i][this.option.selectColumns[0].checked] = false;
                ...
                for (var i = 0; i < this.selectData.length; i++) {
                    if (selectList.indexOf(this.selectData[i][this.option.uniqueId]) >= 0) {
                        this.selectData[i][this.option.unselectColumns[0].checked] = false;
