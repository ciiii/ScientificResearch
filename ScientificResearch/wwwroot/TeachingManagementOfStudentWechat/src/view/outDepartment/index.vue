      ...
      >
        <div v-for="item in list" :key="item.编号" class="box">
          ...
          <div>
           ...
           <div>
            <label style="width:100px;">申请出科日期:</label>
            <span class="red">{{item.申请出科日期}}</span>
          </div>
           <div>
            <label>建立时间:</label>
            <span>{{item.建立时间}}</span>
...
export default {
  ...
  methods: {
    ...
    edit(item) {
      if (!item.编号) {
        this.$router.push("/outDepartment/edit");
      }
    },
    detail(item) {
      this.$router.push(
        "/outDepartment/detail?id=" + item.SourceId + "&name=" + this.title
      );
    },
