      ...
      >
       <div v-for="item in list" :key="item.编号" class="box">
          ...
          <div>
           ...
           <div>
            <label style="width:100px;">轮转手册类型:</label>
            <span class="red">{{item.教学轮转手册类型名称}}</span>
          </div>
          <div>
            <label>建立时间:</label>
            <span>{{item.建立时间}}</span>
          ...
          <div class="actions">
            <van-button plain icon="eye-o" 
            v-show="item.IsCanOperate"
            type="danger" @click="edit(item)" size="small">审核</van-button>
            <van-button
              plain
              icon="orders-o"
              type="primary"
...
export default {
  ...
  methods: {
    ...
    edit(item) {
      if (!item.编号) {
        this.$router.push("/rotaryManual/edit");
      }
    },
    detail(item) {
      this.$router.push(
        "/rotaryManual/detail?id=" + item.SourceId + "&name=" + this.title
      );
    },
