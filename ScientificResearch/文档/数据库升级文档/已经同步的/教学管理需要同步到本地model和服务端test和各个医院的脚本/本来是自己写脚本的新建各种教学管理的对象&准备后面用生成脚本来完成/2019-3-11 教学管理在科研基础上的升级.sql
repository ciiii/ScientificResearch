...
EXEC sys.sp_addextendedproperty @name = N'MS_Description',
   ...
   某个具体的步骤模板,可以有多种状态;
   新生成一个步骤时,状态为0,比如提交一个请假到审核步骤,这个状态叫待审核
   又比如审核步骤时,可以有通过和不通过操作
...
EXEC sys.sp_addextendedproperty @name = N'MS_Description', @value = N'步骤
   ...
   当前步骤的状态用了步骤模板编号+状态编号+排序来说明
   虽然可以用步骤模板状态id来指定,但变换其状态来比较麻烦;', @level0type = N'SCHEMA',
...
AS
    BEGIN
		...
		--如果对于特定的步骤,传入的人的特征符合该步骤指定人的描述,则返回true
        IF ( EXISTS ( SELECT    1
        ...
        ELSE
            IF ( EXISTS ( SELECT    1
                          ...
                          WHERE     a.Id = @StepId ) )
                RETURN 1;
			--如果该步骤模板有一个可操作人的标签为null-0,则该流程发起人=现在传进来的人就可以操作
            ELSE
                IF ( EXISTS ( SELECT    1
                              FROM      dbo.Step a
